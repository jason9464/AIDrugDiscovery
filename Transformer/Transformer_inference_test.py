...
def predict(model, test_loader, gpu): ## 모델 훈련
    ...
    with torch.no_grad():
        ...
        pred_dict = {"pred_label": [int(i) for i in output_list], "prob" : [float(i) for i in prob_list]}
        pd.DataFrame(pred_dict).to_csv("test.csv")
...
if __name__ == '__main__':
    ...
    test_data = pd.read_csv('SARS-CoV2_Omicron-BA1_IC50_20230118.csv')
