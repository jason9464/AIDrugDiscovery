...
def train(args, gpu, criterion, dataset, model_save_path, is_save=False, MAX_LEN=30): ## 모델 훈련
    num_epochs, lr, reg = args.num_epochs, args.lr, args.reg
    
    save_dir = "result_cls_HL/{}_lr{}_epoch{}_reg{}_antigen".format("classification", lr, num_epochs, reg)
    ...
    for fold, (train_index, test_index) in enumerate(kfold.split(dataset, dataset.labels)):
        ...
        model = transformer_classification(26, embed_dim=128, hidden_dim=1024, num_head=4, num_layer=4, drop_prob=0.1, class_num=2, MAX_LEN=MAX_LEN)
        model.to(gpu)
        ...
        for epoch in range(num_epochs):
            ...
            with tqdm(total=len(train_loader)) as pbar:
                ...
                for batch_idx, batch in enumerate(train_loader):
                    ...
                    batch_X_1, batch_X_2, batch_y, batch_antibody, batch_antigen = batch
                ...
                with torch.no_grad():
                    ...
                    for X_1, X_2, y, antibody, antigen in tqdm(test_loader):
                ...
                pred_dict = {"CDRH3": antibody_list, "CDRL3": antigen_list, "pred_label": [int(i) for i in output_list], "Class": [int(i) for i in y_list], "prob" : [float(i) for i in prob_list]}
...
if __name__ == '__main__':
    ...
    model_save_path += 'Transformer_classification_Cov_HL'
    ...
    ## dataset
    df = pd.read_csv('SARS-CoV2_Omicron-BA1_121422.csv', encoding='cp949') # dataset file here
    dataset = FASTADataset(
        sequences1=df.Heavy_chain.to_numpy(),
        sequences2=df.Light_chain.to_numpy(),
    ...
    train(opt, gpu, criterion, dataset, model_save_path, is_save=model_save, MAX_LEN=MAX_LEN)
