...
from data_prep import FASTADataset_one
from model import transformer_classification_one
...
def train(args, gpu, criterion, dataset, model_save_path, is_save=False): ## 모델 훈련
    num_epochs, lr, reg = args.num_epochs, args.lr, args.reg
    
    save_dir = "result_cls_CDRL/{}_lr{}_epoch{}_reg{}_antigen".format("classification", lr, num_epochs, reg)
    ...
    for fold, (train_index, test_index) in enumerate(kfold.split(dataset, dataset.labels)):
        ...
        model = transformer_classification_one(26, embed_dim=128, hidden_dim=1024, num_head=4, num_layer=4, drop_prob=0.1, class_num=2)
        model.to(gpu)
        ...
        for epoch in range(num_epochs):
            ...
            with tqdm(total=len(train_loader)) as pbar:
                ...
                for batch_idx, batch in enumerate(train_loader):
                    ...
                    batch_X_1, batch_y, batch_CDRH = batch
                ...
                with torch.no_grad():
                    ...
                    for X_1, y, CDRH in tqdm(test_loader):
                ...
                # pred_dict = {"CDRH3": antibody_list, "pred_label": [int(i) for i in output_list], "Class": [int(i) for i in y_list], "prob" : [float(i) for i in prob_list]}
                ...
                print("Fold: {}, Epoch: {}, train_loss: {:.4f}, test_acc: {:.4f}".format(fold, epoch, train_loss, acc))
        ...
        precision = precision_score(y_list.data.cpu().numpy(), output_list.data.cpu().numpy(), pos_label=0)
        recall = recall_score(y_list.data.cpu().numpy(), output_list.data.cpu().numpy(), pos_label=0)
        f1 = f1_score(y_list.data.cpu().numpy(), output_list.data.cpu().numpy(), pos_label=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--reg', type=float, default=0.01) ## weight decay
    parser.add_argument('--batch_size', type=int, default=512) ## batch_size
    parser.add_argument('--num_epochs', type=int, default=150) ## num_epochs
    ...
    model_save_path += 'Transformer_classification_Cov_CDRL3'
    ...
    ## dataset
    df = pd.read_csv('SARS-CoV2_Omicron-BA1_121222.csv') # dataset file here
    dataset = FASTADataset_one(
        sequences1=df.CDRL3.to_numpy(),
    ...
    train(opt, gpu, criterion, dataset, model_save_path, is_save=model_save)
