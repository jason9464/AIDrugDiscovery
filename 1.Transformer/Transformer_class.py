...
def evaluate_category(model, test_loader, gpu, criterion, test=False): ## dataset에 대한 evaluation
    ...
    with torch.no_grad():
        ...
        if test == True:
            pred_dict = {"CDRH3": antibody_list, "CDRL3": antigen_list, "pred_label": [int(i) for i in output_list], "Class": [int(i) for i in y_list], "prob": prob_list}
            pd.DataFrame(pred_dict).to_csv("classification_testdataset_result2.csv")
...
def train(args, gpu, criterion, optimizer, model, train_loader, test_loader, is_save=False): ## 모델 훈련
    train_loss_arr = []
    test_loss_arr = []
            ...
            if epoch % 1 == 0:
                with torch.no_grad():
                    ...
                    test_loss, ACC_test = evaluate_category(
                        model, test_loader, gpu, criterion, False)
...
if __name__ == '__main__':
    ...
    model_save_path += 'Transformer_classifier_Cov2'
    ...
    train(opt, gpu, criterion, optimizer, model, train_loader, test_loader, is_save=model_save)
    
    model1 = load_trained_model(model_save_path, gpu)
    
    test_loss, ACC_test = evaluate_category(model1, test_loader, gpu, criterion, True)
