...
def train(args, gpu, criterion, dataset, model_save_path, is_save=False): ## 모델 훈련
    ...
    save_dir = "result_cls/{}_lr{}_epoch{}_reg{}".format("classification", lr, num_epochs, reg)
...
if __name__ == '__main__':
    ...
    model_save_path += 'Transformer_classification_Cov'
    ...
    ## dataset
    df = pd.read_csv('Preprocessed_CoV-AbDab-20221206.csv') # dataset file here
    ...
    # plt.plot(np.arange(1, opt.num_epochs+1), train_loss_arr, 'ro-', linewidth=2)
    # plt.title('Loss function with respect to epoch')
    # plt.xlabel('Epoch')
    # plt.ylabel('Loss')
